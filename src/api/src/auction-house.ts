...
export const show_escrow = async (cmd: any) => {
  //{ env: 'devnet', keypair: 'my-keypair.json' }
...
export const withdraw = async (cmd: any) => {
  const { keypair, env, amount, auctionHouse, auctionHouseKeypair } = cmd;
  const auctionHouseKey = new web3.PublicKey(auctionHouse);
  const walletKeyPair = loadWalletKey(keypair);
  ...
  if (auctionHouseKeypairLoaded) {
    ...
    instruction.keys
      .filter((k: { pubkey: { equals: (arg0: web3.PublicKey) => any } }) =>
        k.pubkey.equals(auctionHouseKeypairLoaded.publicKey)
      )
      .map((k: { isSigner: boolean }) => (k.isSigner = true));
  }

  instruction.keys
    .filter((k: { pubkey: { equals: (arg0: web3.PublicKey) => any } }) =>
      k.pubkey.equals(walletKeyPair.publicKey)
    )
    .map((k: { isSigner: boolean }) => (k.isSigner = true));
...
export const sell = async (cmd: any) => {
  ...
  if (auctionHouseKeypairLoaded) {
    ...
    instruction.keys
      .filter((k: { pubkey: { equals: (arg0: any) => any } }) =>
        k.pubkey.equals(auctionHouseKeypairLoaded.publicKey)
      )
      .map((k: { isSigner: boolean }) => (k.isSigner = true));
  }
...
export const deposit = async (cmd: any) => {
  ...
  var output = {
    deposited: amountAdjusted,
    account: auctionHouse,
    newBalance: currBal + amountAdjusted,
  };
...
export const create_auction_house = async (cmd: any) => {
  ...
  let twdKey: web3.PublicKey, fwdKey: web3.PublicKey, tMintKey: web3.PublicKey;
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
// function setLogLevel(value: string, prev: any) {
